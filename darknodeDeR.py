...
# topic[0] = 0xf73268ea792d9dbf3e21a95ec9711f0b535c5f6c99f6b4f54f6766838086b842
...
try:
    if USETEMPBACKUP:
        df = pd.read_csv(
            os.path.join(dirname,"darknodesDeR.temp.csv"),
            dtype={
                "block": int,
                "transaction": str,
                "owner": str,
...
for i in range(fromBlock, latest, its):
    ...
    t = [t[-1], time()]
    logger.info(f"{round((end - fromBlock) / (latest - fromBlock) * 100)}%, {len(logs)} events to fetch, Est. time {int(((latest - fromBlock) / its) * (1 - (end - fromBlock) / (latest - fromBlock))*(t[1]-t[0]))}")  # Really, why did I spend time estimating this? At least I didn't reduce the equation and instead just went with it. Someone should fix the initial estimation that is completly bonkers.
    for log in logs:
    ...
    df.to_csv(os.path.join(dirname, "darknodesDeR.temp.csv"), index=False)

df.to_csv(os.path.join(dirname, "darknodesDeR.csv"), index=False)
